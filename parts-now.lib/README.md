...
| V8 | Error | Reject `<Substance>`, `<MixtureReaction>`, `<FixedReaction>`, `<ThermalReaction>`, `<GrainGeometry>`, `<Situation>`, `<EditorTagDef>`, `<Explosion>`, `<ExplosionVolume>` as top-level assets | These feed startup-only libraries or registries the runtime loader does not own for rollback/unload. Matched only on the bundle's direct children, so a same-named *reference* nested inside game data is not hit |
...
KSA 5438 introduces top-level Explosion and ExplosionVolume assets. Validation rule V8 rejects these definitions because Parts Now cannot roll back or unload their registries. References nested in supported assets are allowed. Install definitions as a regular mod and restart KSA.
