...
6. Restore in dependency order: global material/planet settings; native-created object ownership and scale baselines; relationships/welds; per-object appearance; placements/cameras; playback configuration. Preflight runtime part-template dependencies before native parsing; external PNG/GLB/audio resources remain explicit dependencies and missing files produce actionable diagnostics. Do not silently import arbitrary paths or recreate missing native vehicles.
...
- Reset old-world mutation queues and active players; no stale GPU/FM0D/viewport ownership; restore repeated effects through normal lifetime APIs. Deferred renderer/physics operations must not run against a later world.
