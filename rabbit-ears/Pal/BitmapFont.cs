...
/// <summary>
/// The classic 5×7 dot-matrix font, ASCII 32..95 (lower case maps to upper case).
/// Five column bytes per glyph, bit 0 = top row.
/// </summary>
public static class BitmapFont
{
    ...
    private static readonly byte[] Columns =
    {
    ...
    /// <summary>True when the glyph for <paramref name="c"/> has a dot at (column, row).</summary>
    public static bool IsSet(char c, int column, int row)
    {
        if (c >= 'a' && c <= 'z') c = (char)(c - 32);
        if (c < ' ' || c > '_') c = '?';
        return ((Columns[(c - ' ') * GlyphWidth + column] >> row) & 1) != 0;
    }

    public static int MeasureWidth(string text, int scale) => text.Length * Advance * scale - scale;
}
