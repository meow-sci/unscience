...
shader ignores material alpha. Managed draw-gating checks cover isolation/restoration; live visual
acceptance remains pending. See [character and materials](character-and-materials.md#kitten-visor-visibility-5402).
...
restrictions. Managed Harmony/persistence checks cover these paths; native flight-computer response
and the detailed acceptance checks remain open in [iron-man.md](iron-man.md). A subsequent orientation
fix supplies headward rocket controls, upright editor camera/pan and geometric backpack RCS mapping.
Full EVA/Iron Man mode buttons now separate native kitten behavior from rocket flight while retaining
editor support in both modes. A scoped surface-teleport adapter now places only active Iron Man kittens
head-up using transformed equipment bounds and native terrain/launchpad logic. Managed isolation,
clearance and queue checks pass; native mode/orientation/teleport acceptance remains open.
...
UV remapping plus conservative box detection and collider/render alignment. Automatic box/mesh
colliders add scoped Bepu contact filtering and bubble lifetime hooks; native collision,
rendering/live-edit/lifetime acceptance remains open in [statics.md](statics.md).
...
- **garrys-torch XYZ scale.** Weld state, UI, presets, animation, and the public API now carry independent
  ...
  scalar TOML/API inputs migrate uniformly. Weld multipliers now preserve captured full-part scales and inherited authored/animated SubPart scales; unweld restores originals, including nondefault kitten size. Managed scale regressions pass; live-check Flexo custom parts, unequal axes, animation and unload restoration.
