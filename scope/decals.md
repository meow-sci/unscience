...
against that build. Touchpoint line numbers are @5438 unless a cell is marked `@5482`; see the
[5482 verification](#ksa-5482-5438--5482-verification) for what moved.
...
| # | Kind | Mod code | Game member | Decomp path (5438 unless `@5482`) | Status | Notes |
|---|---|---|---|---|---|---|
...
| 9 | Direct API (pick) | `DecalPicker.cs:53-56,82-103` | `Cursor.GetEgoRay(IViewport)` (**replaced `Cursor.InputRay` @5402**); `Part.RayCastEgo(ref readonly double4x4, Ray, out …×8, out Part?, out Part?)`; `Vehicle.{BoundingSphereRadiusBody, GetMatrixAsmb2Ego(Camera)}`; `Camera.{GetPositionEgo(IPosition), NearbyCelestial}` | `KSA/Cursor.cs:27` (byte-identical @5482); `KSA/Part.cs:2534` @5482; `KSA/Vehicle.cs:1270` @5482; `KSA/Camera.cs:231,71` | ✅ **fixed @5402** (compile break, `Cursor.InputRay`/`UpdateInputRay`/`ScreenPosition` removed); ⚠ semantic @5482 (compatible) | `GetEgoRay(viewport)` = `viewport.GetCamera().ScreenToEgoRay(Cursor.GetPosition(viewport))` — ego-space, built on demand from the viewport-local cursor position (`DesktopPosition - viewport.Position`) and the **current** camera, so it is same-frame (was one frame stale). Passed `Program.MainViewport` (main-viewport-only feature). RayCastEgo only hits SUB-parts — a top-level part with no sub-parts returns false (the loop never runs); acceptable: stock parts all have sub-parts. **@5482** `RayCastEgo` first rejects rays that miss a whole-part sphere (radius = `\|max corner of BoundingBoxPartAsmb\|` × largest `ScaleTotal` element) before the sub-part loop; signature unchanged, so picking of heavily non-uniformly scaled parts needs a live check. |
...
no native KSA run. **No graffiti code change was needed.** Evidence:
[KSA_5482_UPGRADE](../plans/KSA_5482_UPGRADE.md).

- ✅ **Seam and ordering unchanged.** `RenderTarget.cs`, `GridPass.cs`, `GlobalShaderBindings.cs`,
  `BindlessTextureLibrary.cs`, `Cursor.cs`, `Ray.cs`, `Celestial.cs`, `ChuteClothSystem.cs` and
  `ChuteClothTopology.cs` are byte-identical; `Core/Renderer.cs` is additive only. The main-path
  order in `RenderGame` is the same: translucency → particles → early
  `ResolveAttachments(cb, inResolveDepth: false)` (`Program.cs:4821`) → depth barrier to
  `DepthStencilAttachmentReadWrite` (`:4835`) → gizmos (`:4848`) → final `ResolveAttachments(cb)`
  (`:4849`) → `GridPass.Run` (`:4852`). The new rev-5458 `DepthSampledReadC` barriers (`:4791`,
  `:4796`) sit before `:4835`, so the depth layout at the final resolve is unchanged. Non-main
  viewports still render through `RenderViewport` (resolve at `:4531`, `_renderedViewport` set at
  `:4417`) and are excluded by the identity checks; the editor resolve (`:4972`) is excluded by
  `EditorFlag`.
- ✅ **Shader includes unchanged.** `Common/Camera.glsl`, `Global.glsl`, `Extensions.glsl`,
  `TextureSet.glsl`, `Grid.frag` and the `DefaultAssets.xml` `GridFrag` id are identical. The decal
  fragment is Lambert + ambient only (no BRDF LUT, no `Lighting.glsl`), so the rev-5472 BRDF fix does
  not reach it; decals may look slightly flatter against the now-corrected hull fresnel (cosmetic).
- ⚠ **`Part.RayCastEgo` whole-part sphere early-out** (`Part.cs:2534`, see #9). It is conservative
  for uniform scale and matches the game's own hover pick; recheck picking on heavily per-axis-scaled
  parts (dont-stifle-me).
- ✅ **Canopy anchors.** `Parachute.DrawCanopy` now takes a leading `ViewHandle` (`Parachute.cs:1185`,
  rev 5474), but graffiti never calls it; the fields it reads (`ClothPositionsFront`,
  `AttachLocationPartAsmb`, `CanopyIndex`) and the `BuildLocal2Ego` math it mirrors (`:1226`) are
  unchanged.
- **Native acceptance pending:** one composite per final resolve with MSAA on and off; hull, kitten
  and canopy decals; terrain accurate-radius placement (outstanding since 5402).
